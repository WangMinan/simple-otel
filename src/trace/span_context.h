...
namespace trace
{
  ...
  public:
    ...
    //bool IsValid() { return !trace_id.empty() && !span_id.empty(); }
    bool IsValid() { return !trace_id.empty() && !span_id.empty(); }
    friend std::ostream& operator<<(std::ostream& os, const SpanContext& spancontext);
  };
  ...
  {
  private:
    static std::unordered_map<std::thread::id, std::shared_ptr<SpanContext>> parent_contexts;
    static std::unordered_map<std::thread::id, std::shared_ptr<SpanContext>> current_contexts;
    static std::unordered_map<std::thread::id, std::shared_ptr<Span>> active_spans;
    static std::unordered_map<std::thread::id, std::vector<std::vector<std::shared_ptr<RespContext>>>> resp_contexts;
    static std::unordered_map<std::thread::id, std::shared_ptr<RespContext>> return_context;
    static std::recursive_mutex ServerContextLock;
